...
scratchpads =
	...
	, NS "mixer" (myTerminal ++ " -name mixer -e alsamixer") (appName =? "mixer") med
		...
		med = customFloating $ W.RationalRect (1/4) (1/10) (1/2) (3/5)
...
dzenClick cmd s = printf "^ca(1,%s)%s^ca()" cmd s

myLayoutIcon layout = dzenClick "xdotool key super+space" (dzenColor orange "" (icon layout))
	where
		icon layout = 
...
myPPTitle "" = ""
myPPTitle title = dzenColor fg_light "" " =>  " ++ dzenColor green "" (shorten 64 title)

myLogHook h = dynamicLogWithPP $ defaultPP
	{ ppCurrent  = dzenColor green bg_dark . pad . dzenSwitchWs
	, ppHidden   = dzenColor fg_light "" . pad . dzenSwitchWs
	, ppHiddenNoWindows = dzenColor fg_dark  "" . pad . dzenSwitchWs
	, ppUrgent   = dzenColor red "" . pad . dzenSwitchWs
	, ppWsSep    = ""
	, ppSep      = " "
	, ppTitle    = myPPTitle
	...
	, ppSort     = fmap (.namedScratchpadFilterOutWorkspace) getSortByTag
		...
		where
			dzenSwitchWs s = dzenClick ("xdotool key super+" ++ s) s
...
mySBBG = bg_light
mySBFont :: String
mySBFont = "inclonsolata:pixelsize=11"
mySBH :: Int
mySBH = 20

drawDzenBar :: Int -> Int -> Int -> Int -> String -> String -> String -> String
drawDzenBar x y h w bg fn ta = printf "dzen2 -p -x %d -y %d -h %d -w %d -bg '%s' -fn '%s' -ta %s" x y h w bg fn ta

drawPipeDzenBar :: String -> Int -> Int -> Int -> Int -> String -> String -> String -> String
drawPipeDzenBar cmd x y h w bg fn ta = cmd ++ " | " ++ drawDzenBar x y h w bg fn ta

drawConkyDzenBar :: String -> Int -> Int -> Int -> Int -> String -> String -> String -> String
drawConkyDzenBar cfg x y h w bg fn ta = drawPipeDzenBar ("conky -c " ++ cfg) x y h w bg fn ta

mySB x y w ta = drawDzenBar x y mySBH w mySBBG mySBFont ta
myConkySB cfg x y w ta = drawConkyDzenBar cfg x y mySBH w mySBBG mySBFont ta
myConkyXSB :: String -> Int -> Int -> String
myConkyXSB cfg x y = printf "conky -c %s -x %d -y %d" cfg x y

myTopSB = mySB 0 0 (myW-30) "l"
...
myBRW = 250
myBY = myH-mySBH
myBLSB = myConkySB "~/.conky/dzbl" 0 myBY myBLW "l"
myBMSB = myConkyXSB "~/.conky/bm" myBLW 0
myBRSB = myConkySB "~/.conky/dzbr" (myW-myBRW) myBY  myBRW "r"

-- extraBars = (myBLSB, myBMSB, myBRSB)
